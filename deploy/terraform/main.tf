...
resource "azurerm_resource_group" "TechSummit2018" {
  name     = "${var.rg-name}"
...
module "query_url" {
  ...
  command = "printf $(az storage account show -n ${azurerm_storage_account.webblob.name} -g ${azurerm_resource_group.TechSummit2018.name} --query \"primaryEndpoints.web\" --output tsv | cut -d \"/\" -f 3)"
}
